...
`complete` 会区分内容校验与存储故障：大小、类型、签名或图片解码不合法属于权威失败，资源进入失败清理流程；R2 HEAD/GET 的超时、5xx、网络中断以及对象暂未可见会返回 HTTP 503，并保持 `PENDING_UPLOAD`，前端可安全重试 `complete`，不会删除对象。

媒体删除采用可恢复的两阶段状态机。服务先在独立数据库事务内锁定媒体、重新确认无引用并提交 `DELETING`，然后在事务外删除 Local/R2 对象，最后以第二个事务提交 `DELETED`。对象删除或最终数据库提交失败时，`DELETING` 会保留，后台任务每次按更新时间处理最多 100 条并继续重试；对象删除必须保持幂等。因此发布或备份脚本不得把 `DELETING` 当作已经彻底删除，也不要手工清除这些行。
